...
# no-op: Wear OS is a different shape and nothing here ships one. No kit command
# calls these, so there is no platform to branch on either.
#
...
# The one model every store capture is taken on, in every repo on the kit. Not
...
# hundred, and the model is the app's own choice rather than the kit's pinned
# one. So these are primitives a repo's script drives, not a pass the kit runs.
#
# What the kit does own is the awkward half — which build products directory
