...
platform_use_device() {
  local wanted="$1" udid
  # JSON, not the table: the table's Identifier column is sometimes the
  # CoreDevice UUID and sometimes the hardware UDID (00008150-0011023A11C0401C,
  # 25 characters), and a regex for one never matched the other. The hardware
  # UDID is what comes back, because xcodebuild's `id=` takes nothing else.
  udid="$(xcrun devicectl list devices --quiet --json-output - 2>/dev/null | python3 -c '
import json, sys
...
# The archive is signed for development; the export after it re-signs with a
# distribution identity, and that is the first step of a ship that needs one.
# A Mac without the certificate therefore builds for five minutes and then
# fails inside exportArchive with "No Accounts" — so ask the keychain first,
# which answers in milliseconds.
ios_require_distribution_identity() {
  if security find-identity -v -p codesigning 2>/dev/null |
    grep -q '"Apple Distribution:\|"iPhone Distribution:'; then
    return 0
  fi
  die "no Apple Distribution certificate in the keychain — the archive would
  build and then exportArchive would fail with 'No Accounts'.

  Xcode owns it: Settings ▸ Accounts, sign in, then Manage Certificates
  ▸ + ▸ Apple Distribution. Or install one from the developer portal."
}
...
#
# The signing gate sits here because this is the one function every ship path
# goes through before anything expensive, --dry-run included, and because the
# Mac adapter already refuses from it.
