...
# Gradle chokes on a modern system JDK; Android Studio ships one that usually
# works. Defaulted, not set: the JBR is a moving target — Studio shipped 21 for
# years, then 25, which Gradle 8.13 cannot even parse (it dies with the bare
# version string as the whole error). An explicit JAVA_HOME is the escape hatch
# for the day the bundled one is the broken one.
_ANDROID_STUDIO_JBR="/Applications/Android Studio.app/Contents/jbr/Contents/Home"
if [[ -z "${JAVA_HOME:-}" && -d "$_ANDROID_STUDIO_JBR" ]]; then
  export JAVA_HOME="$_ANDROID_STUDIO_JBR"
fi
...
# The device `appkit run` was told to use, by serial or by the `device:` name adb
# prints beside it. An emulator and a phone on a cable are one list here, the
# same as they are on iOS, so DEVICE_KIND is set for symmetry even though
# nothing on Android branches on it: install and launch are adb either way.
#
# This was missing entirely until it was noticed that run.sh calls it on both
# platforms — `appkit run --device` on Android died with "command not found",
# which is the failure the contract exists to make impossible.
platform_use_device() {
  local wanted="$1" serial
  [[ -x "$ADB" ]] || die "adb not found — install the Android SDK platform-tools"
  serial="$("$ADB" devices -l | awk -v want="$wanted" '
    NR > 1 && $2 == "device" {
      name = ""
      for (i = 3; i <= NF; i++) if ($i ~ /^device:/) { sub(/^device:/, "", $i); name = $i }
      if ($1 == want || name == want) { print $1; exit }
    }')"
  [[ -n "$serial" ]] || die "no adb device named '$wanted' — run: appkit run"
...
# Both numbers live in one file, which is the module's build script. `app/` is
# the module name Android Studio gives the first one and the same assumption
# android.aab already makes; a repo that renamed it says so there and here.
ANDROID_GRADLE="${ANDROID_GRADLE:-$ROOT_DIR/app/build.gradle.kts}"

platform_version() {
  local version
  version="$(awk -F'"' '/versionName/ { print $2; exit }' "$ANDROID_GRADLE" 2>/dev/null)"
  [[ -n "$version" ]] || die "could not read versionName from ${ANDROID_GRADLE#"$ROOT_DIR/"}"
  printf '%s' "$version"
}

# versionName is what the listing shows; versionCode is what Play refuses to see
# twice. An empty version bumps the code alone, which is `appkit version build`.
platform_set_version() {
  local version="$1" current next
  [[ -f "$ANDROID_GRADLE" ]] || die "no build script at ${ANDROID_GRADLE#"$ROOT_DIR/"}"
  current="$(awk '/versionCode/ { print $NF; exit }' "$ANDROID_GRADLE")"
  [[ "$current" =~ ^[0-9]+$ ]] || die "no numeric versionCode in ${ANDROID_GRADLE#"$ROOT_DIR/"}"
  next=$((current + 1))
  if [[ -n "$version" ]]; then
    sed -i '' -E "s/versionName = \"[^\"]+\"/versionName = \"$version\"/" "$ANDROID_GRADLE"
    log "versionName -> $version"
  fi
  sed -i '' -E "s/versionCode = [0-9]+/versionCode = $next/" "$ANDROID_GRADLE"
  log "versionCode -> $next"
}

# Unreachable in practice — an Android repo has no appStore locale row, so
# upload.sh never enters the App Store half — and defined anyway, because a
# contract function that exists in two adapters out of three is the bug this
# whole layout is arranged to prevent.
platform_publish_args() {
  die "Android ships to Play, not to App Store Connect. Remove the appStore
  locale rows from appkit.json, or add a play row to each."
}

# Nothing yet, and the gap is worth naming rather than leaving as silence: Play
# does have a one-time signing setup — the upload keystore — but Play App
# Signing means it is generated once in the Play Console and downloaded, not
# minted locally the way macOS's is. There is nothing for a script to make.
platform_setup_signing() {
  log "Android signing is Play App Signing: the Console holds the app signing key,"
  log "  and your upload key comes from there. Nothing for appkit to set up."
}

# Sideloading an APK is real, and is nothing like the Mac shape this verb has:
# no notary, no ticket, no Gatekeeper — the user turns on a setting. A
# `gradlew assembleRelease` wrapper would be a different command wearing this
# one's name, so it stays unwritten until an app here needs it.
platform_distribute() {
  die "Android distribution outside Play is a signed APK the user sideloads, and
  appkit does not build one. \`appkit ship\` puts an AAB on a Play track."
}

# Nothing. Kotlin is formatted by the toolchain the project already carries, and
# shipping a ktlint config from here would be appkit encoding one app's taste
# in a language it has no other opinion about.
