...
name: release
description:
  Ship a release end to end — version bumps, store screenshot capture and
  delivery, and the listing copy, on the App Store or Google Play. Use when
  cutting a release, regenerating store screenshots, or writing/editing store
  metadata (name, subtitle, keywords, description, what's new).
---
...
iOS: `MARKETING_VERSION` moves every release, `CURRENT_PROJECT_VERSION` every uploaded build, and every target moves together — a build number that differs between an app and its extension is rejected at upload, after the archive. Android: `versionName` and `versionCode`, and Play refuses a code it has already seen.
...
```
appkit release                      # capture, compose, stop
git status -- store/screenshots     # "did any screen change"
```
...
- **Exactly one capture run at a time.** Two runs drive the same devices and interleave: one launches a scene while the other photographs, and the PNG lands under another scene's name in another language's folder. Nothing fails and nothing warns. Check before starting, and before trusting a set someone else kicked off:
  ...
  ```
  ps -Ao pid,lstart,command | grep -E 'appkit (capture|release)' | grep -v grep
  ```
...
- Raw captures are gitignored; the composed cards under `store/screenshots` are **tracked**, because a store screenshot is reviewed in a diff.
...
```
appkit publish copy --pull   # what the store ACTUALLY shows
appkit publish copy --check  # lengths + locale coverage, no upload
git diff -- store/metadata   # what you are about to change
appkit publish copy 1.3.0
```

**Always pull before editing, even for a one-word change.** What comes back is the truth about what the store currently shows, and it is the only thing that tells you whether you are filling a gap or painting over good copy. Never author a metadata tree from scratch without pulling first.

`--check` counts **characters, not bytes** — every CJK locale fails a `wc -c` check that `en-US` passes. An app on both stores is checked against the tighter of the two limits, because copy that fits one and not the other fails a push halfway through, having already changed the locales before it.
...
```
appkit publish shots 1.3.0  # the cards
...
- **The build number is resolved against Connect**, not guessed here. That is the whole reason the archive goes through asc rather than a chain written in the kit — a number picked locally is rejected as a duplicate _after_ the archive, which is the expensive half.
...
- Imagery persists until replaced, which is why upload is opt-in and most releases skip it. `--anyway` is for a version that has never had a set.
