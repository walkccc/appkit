...
store/metadata/
  ...
  description/<language>.txt           the long description
  changelog/<version>/<language>.txt   what changed in one release
...
## Prose belongs in a file

Two of those keys run to paragraphs, and a paragraph inside a JSON string is a line nobody can read or diff. Both have a file of their own that **wins over the JSON key whenever it holds anything**:

| Field         | The file that wins                   |
| ------------- | ------------------------------------ |
| `description` | `description/<language>.txt`         |
| `whatsNew`    | `changelog/<version>/<language>.txt` |

**The extension is not part of the lookup.** `en.txt`, `en.md`, or a bare `en` — whatever the file is called, its text is what goes up. `.txt` is the house spelling because nothing here is ever rendered: the file is uploaded exactly as typed, so a heading written `## What you can do` is a listing that says `## What you can do`. Write what the store should show — a bare line for a heading, `•` for a bullet — and let the blank lines do the rest. If one language somehow has two files, `.txt` wins, then `.md`, then the rest by name.

The file is folded in while a push is staged, so publishing never rewrites the tracked tree, and `appkit check metadata` measures it against the same 4000-character ceiling. `pages/` is the exception and stays `.md`: those are documents a website renders, so there the suffix means something.

Keeping the JSON key as well is harmless, but only one of them ships — delete it once the file exists, or the JSON is a copy that quietly rots.
...
`changelog/<version>/<language>.txt`, one per language, created by hand — the suffix is only the house spelling, any name for the language works:

```sh
mkdir -p store/metadata/changelog/1.3.0
touch store/metadata/changelog/1.3.0/{en,ja}.txt
```
