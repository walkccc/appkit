...
`appkit doctor` prints the model, so "which phone do these come off" is answerable here rather than by reading the kit. It is pinned there and not in `appkit.json` on purpose: one place to move means every repo moves at once, and a per-repo model is a per-repo way to drift silently.
