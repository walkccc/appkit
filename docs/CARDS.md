...
The two numbers are independent: the phone is pinned in the kit and the card size is written here, and changing the phone does not change the card. What the card size _does_ decide is `ios.deviceTypes` in `appkit.json` — 1242×2688 is `IPHONE_65` — and `appkit doctor` checks every size in this file against what the manifest declares, because a mismatch uploads nothing and says it succeeded.
...
```jsonc
{
  ...
  "devices": {
    ...
    // A bare filename is one of the kit's own, in render/bezels/; a path
...
A `frame` with a `/` is resolved against **the repo**, like every other image in a storyboard — `store/art/iphone-17-pro.png` is a repo's own PNG, for a bezel no other app on the kit ships. A bare filename instead resolves against the kit's own `render/bezels/` — `iphone-17-pro-blue.png` and `apple-watch-s11-42mm-silver-forest.png` are photographed once, pinned in the kit the way the capture device itself is, so no repo keeps a copy or writes a path to reach one.
