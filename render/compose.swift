...
//           --out store/screenshots --fonts <kit>/render/fonts --kit <kit>
//
...
let kitRoot = kitDir.isEmpty ? nil : URL(fileURLWithPath: kitDir)
...
/// A `frame` names a bezel PNG. A bare filename — no `/` — is one of the kit's
/// own, in `render/bezels/`: the phone and the watch are each one photograph,
/// pinned in the kit the way the capture device itself is, so no repo keeps a
/// copy or writes a path to reach it. A name with a `/` is the repo's own art,
/// for a bezel no other app on the kit ships.
func resolveFrame(_ path: String) -> String {
  guard !path.contains("/") else { return resolve(path) }
  guard let kitRoot else { fail("\(path) needs --kit") }
  return kitRoot.appendingPathComponent("render/bezels").appendingPathComponent(path).path
}
