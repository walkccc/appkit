# Progress reporting shared by every appkit command. Sourced, never executed.

# What a command says about itself: the banner block at the top of its file,
# between the first ==== rule and the next. Read from the file rather than a
# line range, because a range is one more thing to keep in step and it had
# already drifted twice by the time this replaced it.
appkit_usage() {
  awk '/^# ={10,}/ { if (seen++) exit; next } seen { sub(/^# ?/, ""); print }' "$1"
}
