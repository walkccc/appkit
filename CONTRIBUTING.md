...
**Is this true of every app on the kit?** Not "does it help the app in front of me" — that app can have it in its own repo.
...
Every capability a command needs from a device is a `platform_*` function, and each of `platform/{ios,android}.sh` defines all of them.
...
Group commits by what changed for the repos on the kit, not by the order the work happened in.
...
Only the copies need that second step — a fix inside a command reaches every repo the moment brew upgrades. A repo that never syncs keeps yesterday's `AGENTS.core.md` and skills, which is exactly what `appkit doctor` is for.
