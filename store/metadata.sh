# The listing copy: one tracked tree per app, two stores read out of it.
#
...
for store in stores:
    for field, limit in (APP_STORE if store == "appStore" else PLAY).items():
        # The tighter of the two: copy that fits one store and not the other is
...
def copy_for(folder, language, locale):
...
written = 0
for kind, keep, out in (
    ("app", APP_INFO, dst / "app-info"),
    ("version", VERSION, dst / "version" / version),
):
    for language, locale in rows:
        data = copy_for(src / kind, language, locale)
        ...
        written += 1
print(f"staged {written} file(s) for App Store Connect")
STAGE
...
notes, written = [], 0
for language, locale in rows:
    app = copy_for(src / "app", language, locale)
    version = copy_for(src / "version", language, locale)
    if not app:
        continue
...
STAGE
  _fold_changelog_play "${PUBLISH_VERSION:-}" "$src" "$dst" "${rows[@]}"
}
