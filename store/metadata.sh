...
#                                           what's new
#   store/metadata/description/<language>.txt          the long description
#   store/metadata/changelog/<version>/<language>.txt  what changed, per release
#   store/metadata/pages/<page>/<language>.md          privacy, support, terms
#
# The two prose folders take any extension, or none — see below. `pages/` does
# not: those are the only files here that are rendered.
#
...
# Prose is a file, not a JSON string. A field that runs to paragraphs — the
# description, a release note — is kept beside the JSON as the plain text the
# store will show, where it can be read and edited as itself; the file wins over
# the JSON field of the same name whenever it has anything in it.
#
#   store/metadata/description/<language>.txt          -> description
#   store/metadata/changelog/<version>/<language>.txt  -> whatsNew
#
# The extension is not part of the lookup: `en.txt`, `en.md`, `en` — the store is
# shown the file's text either way, and a rule that reads one and silently skips
# the other costs a release its notes to save nothing. `.md` was the only
# spelling at first, and new trees say `.txt` because nothing here is ever
# rendered and a store listing full of `##` is what markdown gets you — but that
# is a house style, not something this code enforces.
#
...
_fold_prose_asc() {
  local field="$1" label="$2" notes="$3" staged="$4" rows=("${@:5}")
  [[ -d "$notes" ]] || return 0
  python3 - "$field" "$label" "$notes" "$staged" "${rows[@]}" <<'FOLD'
import json, pathlib, sys

field, label = sys.argv[1], sys.argv[2]
notes, staged = pathlib.Path(sys.argv[3]), pathlib.Path(sys.argv[4])
rows = [pair.split("=", 1) for pair in sys.argv[5:]]


def prose(folder, *stems):
    """One field kept as a file, under whatever name. The store is shown plain text
    either way, so the extension is a note to whoever edits the file and nothing
    here reads it. .txt then .md only to break a tie the way the old two-suffix
    lookup did, so a tree carrying both spellings of a language does not change
    what it publishes on the day this landed."""
    rank = {".txt": 0, ".md": 1}
    for stem in stems:
        found = [path for path in folder.glob(f"{stem}.*") if path.is_file()]
        if (folder / stem).is_file():
            found.append(folder / stem)
        for path in sorted(found, key=lambda p: (rank.get(p.suffix, 2), p.name)):
...
for language, locale in rows:
    ...
    # the same precedence the JSON side reads by.
    text = prose(notes, locale, language)
    if not text:
        continue
    ...
    folded += 1
if folded:
    print(f"  {label} for {folded} locale(s)")
FOLD
}
...
for language, locale in rows:
    text = prose(notes, locale, language)
    if text:
        release.append({"language": locale, "text": text})
if release:
...
# The description when it is kept as prose: same field, same ceiling, a file the
# JSON walk above never sees.
folder = root / "description"
# Every file in the folder is one language's description; a dotfile is the
# Finder's, not ours.
stems = {p.stem for p in folder.iterdir() if p.is_file() and not p.name.startswith(".")} \
    if folder.is_dir() else set()
for stem in sorted(stems):
    files += 1
    text = prose(folder, stem)
    if not text:
        continue
    counted += 1
    limit = limits.get("description")
    if limit and len(text) > limit:
        problems.append(f"{stem}/description: {len(text)} > {limit}")
...
STAGE
  if [[ "$scope" == all ]]; then
    _fold_prose_asc description "the description from description/" \
      "$src/description" "$dst/version/$version" "${rows[@]}"
  fi
  _fold_prose_asc whatsNew "what's new from the changelog" \
    "$src/changelog/$version" "$dst/version/$version" "${rows[@]}"
}
...
for language, locale in rows:
    ...
    listing = {
        ...
        "fullDescription": prose(src / "description", locale, language) or version.get("description", ""),
    }
...
for language in languages:
    ...
    if not (has(root / "version" / f"{language}.json", "description")
            or prose(root / "description", language)):
        gaps.append(f"version/{language}: no description")

    if (root / "changelog").is_dir():
        if not prose(root / "changelog" / version, language):
            gaps.append(f"changelog/{version}/{language}: missing")
