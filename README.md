...
6. **Release** with `/appkit-release`: the version bump, the real screenshot
   set, the preview videos, the listing upload, and `appkit ship` — in one pass.
...
| `/appkit-release`              | a whole release in one pass — screenshots, preview videos, every upload, the binary         |
