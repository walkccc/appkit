...
![A contact sheet of 30 store cards across 5 locales, one row per language](docs/contact-sheet.png)
...
uses whatever `adb` gives it and boots nothing.

**Building is yours.** appkit only runs `xcodebuild` or `gradlew` inside `appkit
run`, `appkit ship` and `appkit capture --build`. Everything else reuses what
the IDE already built, which is why capture takes a flag to compile rather than
doing it by default.
